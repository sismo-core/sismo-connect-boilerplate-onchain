...
contract Airdrop is ERC20, SismoConnect {
  ...
  constructor(
    ...
    ERC20(name, symbol)
    SismoConnect(buildConfig(appId, isImpersonationMode)) // <--- Sismo Connect constructor
  {}
  ...
  function claimWithSismo(bytes memory response) public {
    ...
    if (claimed[vaultId]) {
      console.log("Already claimed for vaultId: %s", vaultId);
    ...
    console.log("airdropAmount: %s", airdropAmount);
